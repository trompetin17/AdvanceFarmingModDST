...
function Crop:DoGrow(dt, nowither)
	if not self.inst:HasTag("withered") then
		...
		if shouldgrow then
			...
			--[[ expecific AdvanceFarmingMod ]] --
			if self.grower and self.grower:HasTag("g_house") and (TheWorld.state.iswinter and TheWorld.state.temperature <= 0) then
